...
class OrderExecutionEngine:
    ...
    def _send_mt4_order(self, mt4, pair: str, action: str, volume: float,
                        sl: float, tp: float) -> Dict:
        """Send an order through the provider API and normalize its response."""
        if hasattr(mt4, 'place_order'):
            result = mt4.place_order(
                symbol=pair,
                order_type=action,
                volume=volume,
                stop_loss=sl,
                take_profit=tp,
                comment='ForexTradingController'
            )
        else:
            result = mt4._send({
                'command': 'ORDER',
        ...
        if not isinstance(result, dict):
            return {'success': False, 'error': f'Invalid MT4 response: {result}'}

        if result.get('success') is True:
            return result

        status = str(result.get('status', '')).upper()
        if status in {'OK', 'ACCEPTED', 'EXECUTED', 'FILLED', 'SUCCESS'}:
            return {**result, 'success': True}

        return result
    
    def execute_signal(self, consensus: Dict, market_data: Dict, position_size: float = 1.0, pair: str = None, sl: float = None, tp: float = None):
        ...
        if sl is not None and tp is not None and sl > 0 and tp > 0:
            ...
            logger.info(f"📊 Using provided SL: {sl}, TP: {tp}")
        else:
            # ===== CALCULATE SL AND TP =====
            if action == 'BUY':
        ...
        for attempt in range(max_retries):
            try:
                ...
                if mt4 is not None:
                    result = self._send_mt4_order(mt4, pair, action, volume, sl, tp)
                else:
